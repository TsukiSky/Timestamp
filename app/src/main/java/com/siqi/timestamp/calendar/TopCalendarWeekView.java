...
public class TopCalendarWeekView extends WeekView {
    ...
    protected boolean onDrawSelected(Canvas canvas, Calendar calendar, int x, boolean hasScheme) {
        if (!calendar.isCurrentDay()){
            canvas.drawCircle(x+mItemWidth/2, mItemHeight/2, mItemHeight/2, selectedBlockPaint);
        }
        return true;
    }
...
//                RectF block = new RectF(blockLeft, 0, blockRight, mItemHeight);
//                canvas.drawRoundRect(block, 24f, 24f, todayBlockPaint);
                if (isSelected) {
                    canvas.drawCircle(x + mItemWidth / 2, mItemHeight / 2, mItemHeight / 2, todayBlockPaint);
